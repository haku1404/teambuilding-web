...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const drawWheel = () => {
    ...
    for (let i = 0; i < remainingUnits.length; i++) {
      ...
      // Lấy tên đầu tiên cho gọn, nếu dài thì cắt bớt
      const shortName = remainingUnits[i].split(' ')[0] + (remainingUnits[i].includes('&') ? ' &...' : '');
      ctx.fillText(shortName, radius - 15, 6);
      ctx.restore();
    }
  ...
  window.startWheelSpin = () => {
    ...
    setTimeout(() => {
      ...
      const numSegments = remainingUnits.length;
      const degreesPerSegment = 360 / numSegments;
      const normalizedRot = wheelRotation % 360;
      ...
      // Show Popup
      document.getElementById('room-winner-room-name').textContent = actualRooms[currentRoomIndex].name;
      document.getElementById('room-winner-name').textContent = winner;
      window.openModal('room-winner-modal');
      ...
      if (currentRoomIndex < actualRooms.length) {
        ...
        // Cập nhật lại vòng quay mà KHÔNG chuyển động
        setTimeout(() => {
  ...
  window.closeRoomWinnerModal = () => {
    window.closeModal('room-winner-modal');
  };
